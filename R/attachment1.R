...
separate.result = separate.reg.mean(Nh, SAMPLE$price, SAMPLE$carat, 
...
combined.result = combined.reg.mean(Nh, SAMPLE$price, SAMPLE$carat, 
                        SAMPLE$index, Xbar, alpha, method="Min", beta0=NULL)
print(combined.result)

############################# output ###########################
sink("attach1_result.txt")
cat('This is an application to diamonds data:','\n')
cat('\n')
cat('"carat" is chosen as the related variable','\n')
cat('\n')
